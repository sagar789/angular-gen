...
CkEditorImageBrowser.initEventHandlers = function() {
    ...
    $(document).on('click', '#getImage', function() {
        if (confirm('Are you sure you want to delete this Image?')) {
            var imagname = $(this).parent().next().children().children().attr('src').split('/');
            var imagpop = imagname.pop();
            $.ajax({
                type: 'post',
                url: '/api/unlink_file',
                data: { path: 'public/admin/ckeditor/thumb/', img: imagpop },
            ...
            $.ajax({
                type: 'post',
                url: '/api/unlink_file',
                data: { path: 'public/admin/ckeditor/images/', img: imagpop },
                success: function(res) {}
            });
        }
